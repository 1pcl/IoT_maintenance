...
def compare_individuals(ind1, ind2):
    ...
    # 总是存在的参数
    deviation1 += abs(param1[0] - DEFAULT_PARAM_VALUES['preventive_check_days'])
    deviation2 += abs(param2[0] - DEFAULT_PARAM_VALUES['preventive_check_days'])
    
    # 只有当heartbeat模块被选中时才计算相关参数
    if "heartbeat" in selected_modules1:
        deviation1 += abs(param1[1] - DEFAULT_PARAM_VALUES['frequency_heartbeat'])
        deviation1 += abs(param1[2] - DEFAULT_PARAM_VALUES['heartbeat_loss_threshold'])
    if "heartbeat" in selected_modules2:
        deviation2 += abs(param2[1] - DEFAULT_PARAM_VALUES['frequency_heartbeat'])
        deviation2 += abs(param2[2] - DEFAULT_PARAM_VALUES['heartbeat_loss_threshold'])
...
def solution_sort_key(sol):
    ...
    param_deviation = 0
    param_deviation += abs(param_values[0] - DEFAULT_PARAM_VALUES['preventive_check_days'])
    if "heartbeat" in sol['modules']:
        param_deviation += abs(param_values[1] - DEFAULT_PARAM_VALUES['frequency_heartbeat'])
        param_deviation += abs(param_values[2] - DEFAULT_PARAM_VALUES['heartbeat_loss_threshold'])
