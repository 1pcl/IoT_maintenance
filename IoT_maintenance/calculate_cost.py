...
def calculate_total_cost_with_simulation(wsn_sim, selected_modules,scene_data):
    """计算系统总成本"""
    # 从全局配置中获取参数
    sensorNum = scene_data["sensor_num"]
    developmentPeople = scene_data["development_people"]
    monthly_salary = scene_data["monthly_salary"]
    development_cycle = scene_data["development_cycle"]
    installation_per_cost = scene_data["installation_per_cost"]
    sensorCost = scene_data["per_sensor_cost"]
    per_packet_cost = scene_data["per_packet_cost"]
    per_check_cost=scene_data["per_check_cost"]
    ...
    # 获取仿真结果
    loss_data_count,node_fault,check_count=wsn_sim.run_simulation() #计算代价的时候不需要画图
    frames_per_round = scene_data["frames_per_round"]
